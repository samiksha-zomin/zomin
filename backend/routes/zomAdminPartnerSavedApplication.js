...
app.get("/", (req, res) => {
  let getDataSql = `
  SELECT acc.email, acc.companyname, acc.name, acc.contnum, acc.industry, app.*, pro.location_com, pro.role FROM accounts acc, bp_application app, partner_profile pro WHERE acc.id=app.user_id AND app.user_id=pro.user_id AND app.application=0;
        `;
...
app.patch("/bePartner", (req, res) => {
    ...
    bePartner.forEach((element) => {
      let updateSql = `UPDATE bp_application set status=1, application=1, joining_date=? WHERE application_id = ?`;
  
      database.query(updateSql, [joinDate, element.application_id], (err, updateList) => {
        if (updateList) {
          status = !status;
        }
        if (err) {

...
app.get("/list", validateToken, async (req, res) => {
  ...
  let totalListSql = `
  SELECT *, ivo.id as interviewID FROM interviewOption ivo, opportunity_give oppo WHERE ivo.bp_id=? and ivo.opportunity_id=oppo.oppo_id ORDER BY interviewDate Desc, interviewTime desc;
        `;
...
app.post("/scheduleInterview", validateToken, async (req, res) => {
  ...
  const grabID = req.body.grabID;
    ...
    database.query(rescheduleSql, [interviewID, candidateID, grabID, employerID, initialTimeDate, proposedTimeDate, remarks], (err, rescheduleRes) => {
      if (rescheduleRes) {
        res.json({
            success: "Successfully reschedule here",
          });
      }
      if (err) {
        console.log(err);
